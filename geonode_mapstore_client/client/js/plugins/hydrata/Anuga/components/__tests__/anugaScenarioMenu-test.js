...
describe('AnugaResultsMenuClass (unconnected — rendering logic)', () => {
    ...
    /*
     * TASK-3078 — the per-row "Show max-value rasters" toggle (TASK-2973) is
     * GONE, and so are the wrappers that existed only to host it as a sibling
     * of the row. Every row is a direct child of the menu again; the
     * actionable row is the menu's ONLY button; nothing carries aria-pressed.
     * Counted, not just "absent": a renamed toggle would still fail here.
     */
    it('renders every Results row without a max-value raster toggle', () => {
        const scenarios = [
        ...
        const menu = container.querySelector('.sv-anuga-results-menu');
        expect(menu).toExist();
        expect(menu.children.length).toBe(2);
        const rows = menu.querySelectorAll('.sv-anuga-results-row');
        const dead = menu.querySelectorAll('.sv-anuga-results-row-unavailable');
        expect(rows.length).toBe(1);
        expect(dead.length).toBe(1);
        expect(rows[0].parentNode).toBe(menu);
        expect(dead[0].parentNode).toBe(menu);
        expect(menu.querySelectorAll('button').length).toBe(1);
        expect(menu.querySelectorAll('[aria-pressed]').length).toBe(0);
        expect(menu.querySelectorAll('[data-testid^="anuga-results-row-toggle-"]').length).toBe(0);
        // Nothing in the menu is a toggle or a wrapper of ANY spelling: every
        // direct child IS a row, and no element carries a toggle/wrap class.
        [...menu.children].forEach((el) => {
            expect(el.classList.contains('sv-anuga-results-row') || el.classList.contains('sv-anuga-results-row-unavailable')).toBe(true);
        });
        expect([...menu.querySelectorAll('*')].some((el) => /toggle|wrap/i.test(el.className))).toBe(false);
    });
});
