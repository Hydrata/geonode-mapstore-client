 ...
 * the peak view is reached on request through the playback bar's Max
 * envelope for a store-backed run. TASK-3078 removed the per-row
 * "Show max-value rasters" control TASK-2973 had added beside every row
 * (operator ruling 2026-09-12: a second, competing peak-view control that
 * cluttered every row; a storeless run, or a store baked without an
 * envelope, therefore has no in-app peak view — a new task if ever wanted).
 * Every row is a direct child of the menu again. Mounted by
    ...
    return (
        <div className="sv-menu-rows-container sv-anuga-results-menu">
            ...
            {/* TASK-2715 — the run finished; only its player is missing. A
                ...
                TASK-3078 — since its max-value rasters no longer paint on
                load (TASK-2973) and the per-row show/hide control is gone,
                this run has no in-app peak view; the row says only what it
                can. */}
