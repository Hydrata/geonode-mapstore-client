 ...
 * those generic selectors (`.glyphicon-trash`, `.sv-menu-row-delete-confirm`)
...
describe('TASK-1010 simpleViewMenuRow — primary/secondary toolbar swap', () => {
    ...
    describe('Primary toolbar (.sv-menu-row-toolbar) — vis | zoom | edit | download', () => {
        it('renders download as the 4th glyph in the primary toolbar', () => {
            return mount(baseLayer()).then(() => {
                const primary = container.querySelector('.sv-menu-row-toolbar');
                expect(primary).toExist();
                const glyphs = primary.querySelectorAll('.sv-menu-row-glyph');
        ...
        it('does NOT render the trash glyph inside the primary toolbar', () => {
            return mount(baseLayer()).then(() => {
                ...
                expect(primary.querySelector('.sv-menu-row-delete-confirm')).toNotExist();
            });
        });
    });

    describe('Secondary toolbar (.sv-menu-row-toolbar-secondary) — delete + overlay (+ upload on swamm)', () => {
        it('renders the trash glyph inside the secondary toolbar', () => {
            return mount(baseLayer()).then(() => {
                const secondary = container.querySelector('.sv-menu-row-toolbar-secondary');
                expect(secondary).toExist();
                const trash = secondary.querySelector('.sv-menu-row-glyph.glyphicon-trash');
        ...
        it('renders the always-mounted delete-confirm overlay inside the secondary toolbar', () => {
            return mount(baseLayer()).then(() => {
                const secondary = container.querySelector('.sv-menu-row-toolbar-secondary');
                expect(secondary).toExist();
                const overlay = secondary.querySelector('.sv-menu-row-delete-confirm');
        ...
        it('overlay contains the danger + cancel buttons (always-in-DOM, R04)', () => {
            return mount(baseLayer()).then(() => {
                ...
                expect(overlay.querySelector('.sv-menu-row-delete-confirm-text')).toExist();
            });
        });
    ...
    describe('Aggregate (primary + secondary) — order vis | zoom | edit | download | trash', () => {
        it('the visual reading order, top-to-bottom of the DOM, is vis -> zoom -> edit -> download -> trash', () => {
            return mount(baseLayer()).then(() => {
                const row = container.querySelector('.sv-menu-row');
                expect(row).toExist();
                const glyphs = row.querySelectorAll('.sv-menu-row-glyph');
        ...
        it('adds sv-glyph-disabled + aria-disabled when terrain resource.deleting=true', () => {
            ...
            return new Promise((resolve) => {
                ReactDOM.render(
                    ...
                    container,
                    () => {
                        const trash = container.querySelector('.sv-menu-row-toolbar-secondary .sv-menu-row-glyph.glyphicon-trash');
                        expect(trash).toExist();
