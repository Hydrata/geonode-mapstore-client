 ...
 *
 * Wire contract (this function's output — TASK-2159: the non-selected XOR
 * column is emitted as explicit null, NOT omitted, so a kind-switch actually
 * clears the stale column on the WFS-T UPDATE):
...
export const translateOut = (input) => {
    ...
    if (data && typeof data === 'object') {
        ...
        if (data.kind === DISCRIMINATOR_KIND.TIMESERIES || data.kind === DISCRIMINATOR_KIND.HYDROGRAPH) {
            ...
            // left the stale constant + new id both populated → inf_data_xor CHECK
        ...
        // a hydrograph clears the stale data_timeseries_id.
