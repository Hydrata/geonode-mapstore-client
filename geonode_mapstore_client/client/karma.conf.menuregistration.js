/**
 * Scoped karma config for the three ANUGA suites that had no fast loop
 * (epic TASK-2706 W5): scenarioHeaderActions-test.js, anugaScenarioMenu-test.js
 * and epicRegistrationCompleteness-test.js. Mirrors karma.conf.scenarios.js —
 * inherits all Hydrata performance overrides from karma.conf.single-run.js but
 * overrides the test entry to point at a scoped test-context file.
 *
 * Run with:
 *   cd /opt/geonode-mapstore-client/geonode_mapstore_client/client && \
 *     NODE_OPTIONS=--openssl-legacy-provider npx karma start \
 *     karma.conf.menuregistration.js --single-run
 */
...
const testWebpackPath = path.join(appDirectory, 'tests-menuregistration-scoped.webpack.js');
